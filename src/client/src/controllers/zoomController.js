...
export default class ZoomController {
  ...
  pinch(event) {
    ...
    // Zoom
    let distance = Math.sqrt(Math.pow(event.touches[1].pageX - event.touches[0].pageX, 2) + Math.pow(event.touches[1].pageY - event.touches[0].pageY, 2))
  ...
  setTargetZoom({ zoom, focalPointX, focalPointY, animate = true, redraw = true }) {
    ...
    let baseOffsetX = (this.store.getCanvasWrapRect().width * this.targetZoom - this.store.getCanvasWrapRect().width * this.currentZoom) / 2
    let baseOffsetY = (this.store.getCanvasWrapRect().height * this.targetZoom - this.store.getCanvasWrapRect().height * this.currentZoom) / 2
    ...
    if (focalPointX && focalPointY) {
      ...
      let focalOffsetX = baseOffsetX * (((this.store.getCanvasWrapRect().width * this.currentZoom) / 2 - fx) / ((this.store.getCanvasWrapRect().width * this.currentZoom) / 2))
      let focalOffsetY = baseOffsetY * (((this.store.getCanvasWrapRect().height * this.currentZoom) / 2 - fy) / ((this.store.getCanvasWrapRect().height * this.currentZoom) / 2))
  ...
  redraw({ animate = false }) {
    ...
    // Repeat
    if (animate) {
      if (this.currentZoom != this.targetZoom || this.actualPanX != this.targetPanX || this.actualPanY != this.targetPanY) {
